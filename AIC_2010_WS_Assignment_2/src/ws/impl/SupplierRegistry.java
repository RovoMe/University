 ...
 * for a product.</p>
 */
@WebService(serviceName="SupplierRegistry", targetNamespace="http://infosys.tuwien.ac.at/aic10/ass1/dto/supplierRegistry", portName="SupplierPT")
@SOAPBinding(style=Style.DOCUMENT, use=Use.LITERAL, parameterStyle=ParameterStyle.WRAPPED)
public class SupplierRegistry
{
	private static final String SUPPLIER1_EP_URL = "http://localhost:8088/Supplier1";
	private static final String SUPPLIER2_EP_URL = "http://localhost:8088/Supplier2";
	
	private Map<String, EndpointReferenceType> supplierOfProduct = 
		new Hashtable<String, EndpointReferenceType>();
	 ...
	 */
        @WebMethod(operationName="registerProductWithSupplier", action="urn:RegisterProductWithSupplier")
        @Action(input="urn:RegisterProductWithSupplier")
		...
		throws UnknownProductFault
	{
		...
		if (product != null && supplierService != null && 
				supplierOfProduct.get(product.getId()) == null)
		{
			...
			serviceNameType.setValue(new QName("http://infosys.tuwien.ac.at/aic10/ass1/dto/supplying", "SupplierService", "tns"));
			...
			supplierOfProduct.put(product.getId(), erType);
		}	
	}
	 ...
	 * 		system.
	 */
        @WebMethod(operationName="getSupplierOfProduct", action="urn:GetSupplierOfProduct")
        @Action(input="urn:GetSupplierOfProduct", output="urn:GetSupplierOfProductResponse")
        @WebResult(name="supplierReference")
	public EndpointReferenceType getSupplierOfProduct(Product product) 
		throws UnknownProductFault
	{
        	
		if (product == null || product.getId() == null) {
			System.out.println("[SupplierRegistry] getSupplierOfProduct: product or productId must not be null");
			throw new UnknownProductFault("Product must not be null");
		}
		Product storedProduct = DataBackend.getInstance().getProduct(product.getId());
		
		if (storedProduct == null) {
			System.out.println("[SupplierRegistry] getSupplierOfProduct: product not found " + product.getId());
			throw new UnknownProductFault("Product is not known to the system");
		}
		
		System.out.println("[SupplierRegistry] checking supplier for: " + product.getId());
		
		EndpointReferenceType erType = supplierOfProduct.get(product.getId());		
		if (erType == null)
			throw new UnknownProductFault("No supplier could be found for product: '" + storedProduct.getName()+"'");

		System.out.println("[SupplierRegistry] supplier found!");
